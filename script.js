...
// Örnek Veri
const hlData = [
    { name: "YouTube İzlenme", value: 5000, desc: "Milyon" },
    { name: "Twitter Takipçi", value: 2500, desc: "Bin" },
    { name: "Netflix Abone", value: 8000, desc: "Milyon" }
];

let leftItem, rightItem;

function startHigherLower() {
    showScreen('game-higherlower'); // Senin mevcut fonksiyonun
    leftItem = hlData[0];
    rightItem = hlData[1];
    updateHLUI();
}
...
function updateHLUI() {
    ...
    document.getElementById('left-value').innerText = leftItem.value + " " + leftItem.desc;
    
    document.getElementById('right-name').innerText = rightItem.name;
    document.getElementById('right-desc').innerText = rightItem.desc;
...
function makeHLGuess(choice) {
    const correct = (choice === 'higher' && rightItem.value > leftItem.value) || 
                    (choice === 'lower' && rightItem.value < leftItem.value);
    
    document.getElementById('right-value').innerText = rightItem.value + " " + rightItem.desc;
    ...
    setTimeout(() => {
        if(correct) {
            leftItem = rightItem;
            rightItem = hlData[Math.floor(Math.random() * hlData.length)];
            updateHLUI();
        } else {
            alert("Kaybettin!");
