...
function renderCats() {
    const cats = ['genelkultur','tarih','matematik','bilim'];
    document.getElementById('cat-grid').innerHTML = cats.map(c => `<button onclick="selectedCat='${c}';renderCats()" class="cat-btn ${selectedCat===c?'active':''}">${c.toUpperCase()}</button>`).join('');
}
function connectToFriend() { conn = peer.connect(document.getElementById('peer-id').value); isHost = false; listen(); }
function listen() {
